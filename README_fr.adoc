...
=== 2. Appliquer et Configurer le Plugin

Maintenant, appliquez le plugin dans votre `build.gradle.kts` et pointez-le vers votre fichier de configuration YAML.

reporter la version_actuel dans le build script gradle: {bakery-version}
...
plugins {
    // Appliquer le plugin
    id("com.cheroliv.bakery") version version_actuel
}
...
Le processus d'intégration continue nécessite un secret GitHub nommé `CHEROLIV_MIGRATION_CONFIG`. Le contenu de ce secret doit être identique à celui de votre fichier de configuration `site.yml`. Le workflow utilise ce secret pour générer le fichier `site.yml` nécessaire à la tâche `publishSite` lors de l'exécution.
...
4.  Pour le **Name**, saisissez `CHEROLIV_MIGRATION_CONFIG`.
