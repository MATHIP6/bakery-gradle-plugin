...
class TempDirBakeryPluginFunctionalTest {

    @Suppress("FunctionName")
    @Test
    fun validate_project_dir_by_output() {
        val projectDir = File("")
            .absoluteFile
            .parentFile
            .parentFile
...
//        logger.info("prepare test environment")
        ...
        "projectDir.listFiles().size: ${projectDir.listFiles().size}"
            .apply(logger::debug)
            .apply(::println)

        assertThat(buildFile).doesNotExist()
        assertThat(buildFile.path)
            .describedAs("buildFile path should be projectDir.path + File.separator + $BUILD_FILE")
            .isEqualTo("${projectDir.path}$separator$BUILD_FILE")

        assertThat(settingsFile).doesNotExist()
        assertThat(settingsFile.path)
            .describedAs("settings File path should be projectDir.path + File.separator + $SETTINGS_FILE")
            .isEqualTo("${projectDir.path}$separator$SETTINGS_FILE")

        assertThat(libsVersionsTomlFile).doesNotExist()
        assertThat(libsVersionsTomlFile.path)
            .describedAs("libs.versions.toml File path should be projectDir.path + File.separator + $GRADLE_DIR${separator} + $LIBS_VERSIONS_TOML_FILE")
            .isEqualTo("${projectDir.path}$separator$GRADLE_DIR$separator$LIBS_VERSIONS_TOML_FILE")

        assertThat(configFile).doesNotExist()
        assertThat(configFile.path)
            .describedAs("app config File path should be projectDir.path + File.separator + $CONFIG_FILE")
            .isEqualTo("${projectDir.path}$separator$CONFIG_FILE")

        // Before creating files
        assertThat(projectDir)
            .describedAs("Before creating files projectDir should be an empty directory")
            .isEmptyDirectory


        // Create files

        // create site.yml files and add its content into and assert validations tests against
        ...
        assertThat(configFile)
            .describedAs("Config file should be created")
            .exists()
            .describedAs("Config file is a physical file")
            .isFile
            .describedAs("Config file should not be empty")
            .isNotEmpty

        assertThat(configFile.readText(UTF_8))
            .describedAs("configFile should contains the given list of strings")
            .contains(configListOfStringContained)

        assertThat(projectDir)
            .describedAs("projectDir should not be an empty directory")
            .isNotEmptyDirectory

        val configSearchedFile = projectDir
            .listFiles()
            .findLast { it.name == "site.yml" }!!
        ...
        val configSearchedFileContent = configSearchedFile.readText(UTF_8)

        assertThat(configSearchedFileContent)
            .describedAs("ConfigContent should contains this list of strings")
            .contains(configListOfStringContained)


        // TODO: create gradle directory
        ...
        // TODO: create gradle/libs.versions.toml files
        libsVersionsTomlFile.writeText(libsVersionsTomlFileContent)

        assertThat(projectDir)
            .describedAs("projectDir should not be an empty directory now that libsVersionsTomlFile is created and written")
            .isNotEmptyDirectory
        ...
        val tomlSearchedFileContent = tomlSearchedFile!!.readText(UTF_8)

        assertThat(tomlSearchedFileContent)
            .describedAs("toml file content should contains this list of strings")
            .contains(tomlListOfStringContained)

        // TODO: create settings.gradle.kts files and add its content into
        settingsFile.writeText(settingsContent)

        assertThat(projectDir)
            .describedAs("projectDir should not be an empty directory now that settingsFile is created and written")
            .isNotEmptyDirectory
        ...
        val settingsSearchedFileContent = settingsSearchedFile!!.readText(UTF_8)

        assertThat(settingsSearchedFileContent)
            .describedAs("Settings content should contains this list of strings")
            .contains(settingsListOfStringContained)

        // TODO: create build.gradle.kts files and add its content into
        buildFile.writeText(buildScriptContent)

        assertThat(projectDir)
            .describedAs("projectDir should not be an empty directory now that buildFile is created and written")
            .isNotEmptyDirectory
    ...
    companion object {
        @Suppress("unused")
        private val logger = getLogger(TempDirBakeryPluginFunctionalTest::class.java)
...
//            logger.info("Initialisation")
            ...
            val sourceConfigFile = File(CONFIG_PATH)
            assertThat(sourceConfigFile)
                .describedAs("Source config file '$CONFIG_PATH' not found.")
                .exists()
                .isNotEmpty
            configContent = sourceConfigFile.readText(UTF_8)
            assertThat(configContent)
                .describedAs("ConfigContent should contains this list of strings")
                .contains(configListOfStringContained)
            // Set and check settings.gradle.kts initialisation
            val gradleSettingsFile = File(SETTINGS_FILE_PATH)
            assertThat(gradleSettingsFile)
                .describedAs("Gradle settings file '$SETTINGS_FILE_PATH' not found.")
                .exists()
                .isNotEmpty
            settingsContent = gradleSettingsFile.readText(UTF_8)
            ...
            // Set and check build.gradle.kts initialisation
            val gradleBuildScriptFile = File(BUILD_FILE_PATH)
            assertThat(gradleBuildScriptFile)
                .describedAs("Gradle build script file '$BUILD_FILE_PATH' not found.")
                .exists()
                .isNotEmpty
            buildScriptContent = gradleBuildScriptFile.readText(UTF_8)
            assertThat(buildScriptContent)
                .describedAs("Gradle build script file should contains build logik")
                .contains(buildScriptListOfStringContained)
            // Set and check libs.versions.toml initialisation
            val libsVersionsTomlFile = File(LIBS_VERSIONS_TOML_FILE_PATH)
            assertThat(libsVersionsTomlFile)
                .describedAs("libs.versions.toml file '$LIBS_VERSIONS_TOML_FILE_PATH' not found.")
                .exists()
                .isNotEmpty
            libsVersionsTomlFileContent = libsVersionsTomlFile.readText(UTF_8)
            assertThat(libsVersionsTomlFileContent)
